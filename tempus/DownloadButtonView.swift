...
class DeformationButton: UIControl {
    ...
    var spinnerView:LoadingSpinner!
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        initSettingWithColor(color: self.tintColor)
    }
    
    func imageWithColor(color:UIColor, cornerRadius:CGFloat) -> UIImage{
        let rect = CGRect(x: 0, y: 0, width: cornerRadius*2+10, height: cornerRadius*2+10)
    ...
    func initSettingWithColor(color:UIColor) {
        ...
        self.spinnerView = LoadingSpinner(frame: CGRect(x: 0, y: 0, width: defaultH*0.8, height: defaultH*0.8))
        ...
        self.addTarget(self, action: #selector(DeformationButton.loadingAction), for: UIControlEvents.touchUpInside)
        
        self.forDisplayButton = UIButton(frame: self.bounds)
        self.forDisplayButton.isUserInteractionEnabled = false
        
        let image = imageWithColor(color: color, cornerRadius: 3)
        self.forDisplayButton.setBackgroundImage(image.resizableImage(withCapInsets: UIEdgeInsetsMake(10, 10, 10, 10)), for: UIControlState.normal)
    ...
    func startLoading(){
        if (btnBackgroundImage == nil) {
            btnBackgroundImage = self.forDisplayButton.backgroundImage(for: UIControlState.normal)
        }
        ...
        self.forDisplayButton.setBackgroundImage(nil, for: UIControlState.normal)
                ...
                UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 0, options: UIViewAnimationOptions.curveLinear, animations: { () -> Void in
                    ...
                    if !self._isLoading {
                        if (self.btnBackgroundImage != nil) {
                            self.forDisplayButton.setBackgroundImage(self.btnBackgroundImage, for: UIControlState.normal)
                        }
                        self.bgView.isHidden = true
                    }
                }
